...
def parse_page(html, want_date):
    ...
    for node in soup.descendants:
        ...
        elif "級別" in lab and "cls" not in L[0]:
            for i, td in enumerate(vals):
            ...
            if key:
                for i, td in enumerate(vals):
                    ...
                    try:
                        ...
                        L[i][key] = None
        elif blk[0] == "setsu" and lab.endswith("ST"):
            for i, td in enumerate(vals):
                t = norm(td.get_text())
                try:
                    ...
                    L[i]["st_setsu"] = None
        elif blk[0] == "setsu" and ("1着率" in lab or "2連率" in lab
                                    or "3連率" in lab):
            key = ("win" if "1着率" in lab
                   else "ren2" if "2連率" in lab else "ren3")
            for i, td in enumerate(vals):
                t = norm(td.get_text())
                try:
                    L[i]["s_" + key] = float(t)
                    ...
                    L[i]["s_" + key] = None
        elif lab.endswith("順位") or lab == "順位":
            for i, td in enumerate(vals):
                t = norm(td.get_text())
