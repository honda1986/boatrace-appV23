...
def notify(topic, r, venue, app_url, at=None):
...
def main():
    ...
    need_o = [(v, r) for v in venues for r in v["races"]
              if r.get("cand") and not r.get("buy") and not r.get("result")]
    if need_o:
    ...
    for v in venues:
        for r in v["races"]:
            ...
            if (when - now).total_seconds() < 60:
                ...
                skip += 1
                continue
            if notify(topic, r, v["venue"], args.app_url,
                      at=when.timestamp()):
                r["notified"] = True
                sent += 1
