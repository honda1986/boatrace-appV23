...
def notify(topic, r, app_url, venue="", at=None):
    ...
    payload = {"topic": topic,
               "title": f"勝負 {venue} {r['rno']}R  "
                        f"ネット{r.get('net') or r.get('close')}締切",
...
def main():
    ...
    ap.add_argument("--app-url", dest="app_url",
                    default=os.environ.get(
                        "APP_URL",
                        "https://honda1986.github.io/boatrace-appV23/"),
                    help="ntfy通知をタップしたときに開くURL")
    args = ap.parse_args()
    ...
    for v in venues:
        for r in v["races"]:
            ...
            if (when - now).total_seconds() < 60:
                ...
                continue
            try:
                ok = notify(topic, r, args.app_url,
                            venue=v["venue"], at=when.timestamp())
            except Exception as e:
                print(f"  通知スキップ {v['venue']}{r['rno']}R "
                      f"{type(e).__name__}: {e}")
                ok = False
            if ok:
                r["notified"] = True
