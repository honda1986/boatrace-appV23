...
REL = ["n_win", "tok", "m_2ren", "avg_st", "st_setsu", "c_win", "mot_pure"]

# モーター純度(motor.py が Kファイルから作る)
MOTOR = {}


def fetch_motor(path="motor/latest.json"):
    """登録番号 -> モーター純度。無ければ空(欠損として扱われる)"""
    try:
        j = json.load(open(path, encoding="utf-8"))
        v = {str(k): float(x) for k, x in (j.get("values") or {}).items()}
        print("モーター純度 {} {}人".format(j.get("date"), len(v)))
        return v
    except Exception as e:
        print("モーター純度が取れません {}".format(type(e).__name__))
        return {}
...
SHOBU_TH = 0.553       # 上位8組の確率合計。全体の約10%
BUY_P = 0.05           # この確率以上の組を買う(実測: 上位8点固定より+3pt)
PAY_LO, PAY_HI = 15000, 20000   # 払戻(1万円換算)がこの範囲のときだけ勝負
BUDGET = 10000         # 配分の計算に使う予算(表示は割合)
NOTIFY_BEFORE = 5        # ネット締切の何分前に通知するか(ntfyの予約送信)
# 通知は2回送る。ntfyの予約はキャンセルできないため。
#   1回目(保険) … 見つけ次第すぐ予約。朝のオッズなので金額はあてにならない
#   2回目(本命) … 締切30分前ごろのオッズで予約し直す。こちらを使う
FINAL_LO, FINAL_HI = 22, 38   # 締切までこの分数のときが「本命」の予約どき
ODDS_MAX_AGE = 12        # オッズがこの分数より古ければ取り直す
ODDS_HORIZON = 24 * 60   # 締切までこの分数以内のレースのオッズを取る
                         # 朝の時点で全候補を取り、通知を予約できるようにする
...
def parse_odds3t(html):
    ...
    for t in soup.find_all("table"):
        k = len(re.findall(r">\s*\d+\.\d\s*<", str(t)))
        if k > best:
            best, tb = k, t
    if tb is None:
        return None
...
def fetch_odds3t(sess, jcd, rno, date):
    try:
        r = sess.get(f"{BASE}/odds3t",
                     params={"rno": rno, "jcd": f"{jcd:02d}", "hd": date},
                     timeout=25)
        r.raise_for_status()
        r.encoding = "utf-8"
        return parse_odds3t(r.text)
    except requests.RequestException:
        return None


def make_buy(combos, odds):
    """確率BUY_P以上の組を、払戻が均等になるよう配分する。
    戻り値: (買い目, 1万円あたりの払戻X) / 条件を満たさなければ None"""
    sel = [z for z in combos if z["p"] >= BUY_P]
    if not sel:
        return None, None
    od = []
    for z in sel:
        o = odds[CIX[z["c"]]] if odds else None
        if not o or o <= 0:
            return None, None
        od.append(o)
    ...
    X = BUDGET / inv.sum()
    w = inv / inv.sum()
    buy = [{"c": z["c"], "p": z["p"], "o": round(float(o), 1),
            "w": round(float(x), 4), "y": int(round(BUDGET * x / 100)) * 100}
           for z, o, x in zip(sel, od, w)]
    return buy, float(X)
...
def make_matrix(race, feats):
    ...
    d["mot_pure"] = np.array([MOTOR.get(str(x.get("toban")), np.nan)
                              for x in L], dtype=float)
...
def notify(topic, r, venue, app_url, at=None, final=False):
    """at を渡すと ntfy の予約送信になる。朝のうちに全部登録できる。
    venue(場名) はレース側に持っていないので、呼び出し元から渡す。"""
    if not topic:
        return False
    buy = r.get("buy") or []
    lines = [f"{z['c']}  {z['o']:.1f}倍  {z['y']:,}円" for z in buy]
    body = ("\n".join(lines) +
            f"\n計 {sum(z['y'] for z in buy):,}円 → 当たれば約{r.get('X', 0):,}円"
            f"\nオッズ {r.get('odds_at') or '—'} 時点"
            + ("" if final else "。締切30分前に本番の通知が届きます")
            + "\n実測 的中率53% 回収率89.6%")
    payload = {"topic": topic,
               "title": (("勝負" if final else "勝負(仮)")
                         + f" {venue} {r['rno']}R  "
...
def main():
    ...
    for jcd, page in open_v:
        ...
        for race in build_rows(page, jcd, {}):
            ...
            # 深夜など、コース別がまだ公開されていない時間に取った記録は
            # 全艇 cwin=0 になる。これを引き継ぐと一日中0のままになるので捨てる。
            if (keep and keep.get("boats") and not keep.get("result")
                    and not any(b.get("cwin") for b in keep["boats"])):
                keep = None
            ...
            races.append({
                ...
                "combos": [{"c": c, "p": round(float(v), 4)}
                           for c, v in combos[:8]],
                "combos_all": [{"c": c, "p": round(float(v), 4)}
                               for c, v in combos if v >= BUY_P],
                "sum8": round(float(sum(v for _, v in combos[:8])), 4),
                "cand": bool(sum(v for _, v in combos[:8]) >= SHOBU_TH),
                "shobu": False,          # オッズを見てから確定する
                "notified": False,
            })
    ...
    for v in venues:
        for r in v["races"]:
            res = r.get("result")
            buy = [z["c"] for z in (r.get("buy") or [])]
            if res and (("rank" not in res) or
                        (buy and not res.get("by_buy"))):
                cl = buy or [z["c"] for z in (r.get("combos") or [])]
                if not cl:
                    continue
                res["rank"] = cl.index(res["hit"]) + 1 if res["hit"] in cl else None
                res["n_combos"] = len(cl)
                res["by_buy"] = bool(buy)
                if buy and res["hit"] in buy:
                    z = r["buy"][buy.index(res["hit"])]
                    res["ret"] = int(round(z["y"] * z["o"]))
                    res["cost"] = sum(q["y"] for q in r["buy"])
                fixed += 1
    if fixed:
    ...
    if any(not r.get("result") for v in venues for r in v["races"]):
        ...
        for v in venues:
            for r in v["races"]:
                ...
                # 買い目があればそれで、無ければ上位8組で判定する
                buy = [z["c"] for z in (r.get("buy") or [])]
                cl = buy or [z["c"] for z in r.get("combos", [])]
                hr = cl.index(x["hit"]) + 1 if x["hit"] in cl else None
                ret = None
                if buy and x["hit"] in buy:
                    z = r["buy"][buy.index(x["hit"])]
                    ret = int(round(z["y"] * z["o"]))
                r["result"] = {
                    "hit": x["hit"], "pay": x["pay"], "henkan": x["henkan"],
                    "won": int(x["hit"].split("-")[0]) == r["top_lane"],
                    "rank": hr, "n_combos": len(cl),
                    "by_buy": bool(buy), "ret": ret,
                    "cost": sum(z["y"] for z in (r.get("buy") or [])) or None,
    ...
    # 4.5) 候補レースのオッズを取って、勝負レースを確定する
    #      条件: 確率5%以上の組を払戻均等で買ったとき、
    #            1万円あたりの払戻が 15,000〜20,000円
    #      オッズは締切が近づくほど動くので、古くなったら取り直す
    need_o = []
    for v in venues:
        for r in v["races"]:
            if not r.get("cand") or r.get("result"):
                continue
            left = mins_left(r.get("net") or r.get("close"), now)
            if left is None or left < 1 or left > ODDS_HORIZON:
                continue
            age = mins_left(r.get("odds_at"), now)
            # odds_at は取得時刻なので、経過分は -age になる
            fresh = (r.get("odds_at") and age is not None
                     and -age < ODDS_MAX_AGE)
            # 本命の予約どきは、多少新しくても必ず取り直す
            if fresh and not (FINAL_LO <= left <= FINAL_HI
                              and not r.get("notified_final")):
                continue
            need_o.append((v, r))
    if need_o:
        print(f"\n候補 {len(need_o)}レースのオッズを取ります "
              f"(締切{ODDS_HORIZON}分前以内 / {ODDS_MAX_AGE}分より古いもの)")
        t1 = time.time()
        with ThreadPoolExecutor(max_workers=args.workers) as ex:
            got = list(ex.map(
                lambda vr: fetch_odds3t(sess, vr[0]["jcd"], vr[1]["rno"], date),
                need_o))
        ok = fail = 0
        for (v, r), od in zip(need_o, got):
            if od is None:
                fail += 1
                continue
            allc = r.get("combos_all") or r["combos"]
            buy, X = make_buy(allc, od)
            if buy is None:
                continue
            was = r.get("shobu")
            r["X"] = round(X)
            r["buy"] = buy
            r["odds_at"] = now.strftime("%H:%M")
            r["shobu"] = bool(PAY_LO <= X < PAY_HI)
            if was is not None and was != r["shobu"]:
                print(f"    {v['venue']}{r['rno']}R  "
                      f"{'勝負に昇格' if r['shobu'] else '勝負から外れた'}"
                      f" (払戻 {X:,.0f}円)")
            ok += 1
        print(f"  取得 {ok}件 / 失敗 {fail}件  {time.time()-t1:.0f}秒")

    # 5) 勝負レースを ntfy に予約登録する(締切の5分前に届くよう指定)
    topic = os.environ.get("NTFY_TOPIC", "").strip()
    sent = final_sent = skip = 0
    for v in venues:
        for r in v["races"]:
            if not r.get("shobu"):
                continue
            ...
            if not t:
                continue
            left = mins_left(t, now)
            if left is None:
                continue
            hh, mm = (int(z) for z in t.split(":"))
            when = now.replace(hour=hh, minute=mm, second=0,
                               microsecond=0) - timedelta(minutes=NOTIFY_BEFORE)
            wait = (when - now).total_seconds() / 60

            # 2回目(本命): 締切30分前ごろ。新しいオッズで送り直す
            if FINAL_LO <= left <= FINAL_HI and not r.get("notified_final"):
                if wait >= 1 and notify(topic, r, v["venue"], args.app_url,
                                        at=when.timestamp(), final=True):
                    r["notified_final"] = True
                    r["notified"] = True
                    final_sent += 1
                    print(f"  本命 {v['venue']}{r['rno']}R  "
                          f"{when.strftime('%H:%M')} に配信 "
                          f"(オッズ {r.get('odds_at')} 時点)")
                continue

            # 1回目(保険): 見つけ次第すぐ。古いオッズでも取りこぼさないため
            if r.get("notified"):
                continue
            if wait < 1:
                ...
                continue
            if left <= FINAL_HI:
                continue                  # 本命の窓が近いので保険は送らない
            if notify(topic, r, v["venue"], args.app_url, at=when.timestamp()):
                r["notified"] = True
                sent += 1
                print(f"  保険 {v['venue']}{r['rno']}R  "
                      f"{when.strftime('%H:%M')} に配信 ({t}締切)")
    ns = sum(1 for v in venues for r in v["races"] if r.get("shobu"))
    nf = sum(1 for v in venues for r in v["races"] if r.get("notified_final"))
    print(f"\n勝負レース {ns}件 / 保険 {sent}件 / 本命 {final_sent}件"
          f" (本命は累計{nf}件)"
